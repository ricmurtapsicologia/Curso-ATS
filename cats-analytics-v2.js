(() => {
  ...
  const send=(event,contentId="",useBeacon=false)=>{ if(IS_AUTOMATION||!page)return false; touchSession(session); const payload=payloadFor(event,contentId); if(useBeacon&&navigator.sendBeacon){try{return navigator.sendBeacon(ENDPOINT,JSON.stringify(payload));}catch{}} try{void ROOT.fetch(ENDPOINT,{method:"POST",mode:"cors",credentials:"omit",cache:"no-store",keepalive:true,referrerPolicy:"strict-origin-when-cross-origin",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)}).catch(()=>{});return true;}catch{return false;} };
